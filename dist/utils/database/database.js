...
class Database {
    constructor() {
        ...
        **/
        ...
        this.insertToDo = async (object) => {
            const { sessionId, ...organizedObject } = objects_1.organizeToDoObject(object);
        ...
        /* Insert a to do adding the user's email as a future guidance. */
        this.updateToDoByTitle = async ({ sessionId, title, newTitle }) => {
            const { email } = await this.getUserBySessionId(sessionId);
            /* Update children */
        ...
        this.removeToDoByTitle = async ({ email, title }) => {
            await this.collections.toDos.deleteOne({ email, title });
            await this.collections.toDos.deleteMany({ email, parent: title });
        };
        this.updateStylesByTitle = async ({ email, title, isCompleted, isFavorite, styles }) => await this.collections.toDos.updateOne({ email, title }, { $set: { isCompleted, isFavorite, styles } });
