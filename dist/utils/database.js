...
class Database {
    constructor() {
        this.connect = async () => this.client.isConnected() ? console.log('Already Connected!') : await this.client.connect();
        /**
        ...
        **/
        ...
        this.isEmailAlreadyRegistered = async (email) => await this.collections.registeredUsers.findOne({ email });
        /* Will insert the user already passing the sessionId, and for so, already logging the user in. */
        this.insertUser = async ({ email, password, sessionId }) => await this.collections.registeredUsers.updateOne({ email }, { $set: { email, password, sessionId } }, { upsert: true });
        /* Used on the login.ts file being responsible for updating and adding the sessionId being guided by the email. */
        this.addSession = async (email, sessionId) => await this.collections.registeredUsers.updateOne({ email }, { $set: { sessionId } });
        /* It will return the entire user object ( email, password and sessionId ) being guided by the sessionId. ( or null if not found ) */
        this.getUserBySessionId = async (sessionId) => await this.collections.registeredUsers.findOne({ sessionId });
        /* Returns all the to Dos of the user. The to Dos are organized with the email being the "owner" of the to Do. */
        this.getToDosByEmail = (email) => this.collections.toDos.find({ email });
        /* A business rule is that the same title cannot be used twice. */
        this.isToDoTitleAlreadyBeingUsed = async (title) => await this.collections.toDos.findOne({ title });
        /* Insert a to do adding the user's email as a future guidance. */
        this.updateToDoByTitle = async ({ email, title, newTitle, newDeadline }) => await this.collections.toDos.updateOne({ email, title }, { $set: { title: newTitle, deadline: newDeadline } });
        /* Remove a to do being guided by it's title. Only possible because of business rule. */
        this.removeToDoByTitle = async ({ email, title }) => await this.collections.toDos.deleteOne({ email, title });
        /* Insert a to do adding the user's email as a future guidance. */
        this.insertToDo = async ({ email, title, deadline }) => await this.collections.toDos.updateOne({ email, title }, { $set: { title, deadline } }, { upsert: true });
        this.client = new mongodb_1.MongoClient(process.env.DATABASE_URI, { useUnifiedTopology: true, useNewUrlParser: true });
        this.controller();
    }
    async controller() {
        /**
